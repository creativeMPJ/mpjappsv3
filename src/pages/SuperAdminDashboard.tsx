...
import { LayoutDashboard, Users, FileText, Settings, LogOut, Shield, Building2, DollarSign, UserCheck, Activity, Menu, Layers } from "lucide-react";
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              {quickActions.map((action) => (
                <Button key={action.label} variant="outline" className="gap-2" onClick={() => handleQuickAction(action.action)}>
              ...
              <TableBody>
                {recentActivity.map((activity) => (
                  <TableRow key={activity.id}>
                    ...
                    <TableCell>
                      <Badge variant={activity.status === "success" ? "default" : activity.status === "pending" ? "secondary" : "destructive"}>{activity.status}</Badge>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
